...
The repository includes a comprehensive test suite under the `testCases/` directory to validate lexical analysis, expression precedence, type variant promotions, and custom language quirks.

To run a specific test case, compile the interpreter and pass the target file path:

**Windows (MinGW/GCC):**
```bash
# Compile the core engine, parser, lexer, and type variant logic
g++ main.cpp parser.cpp lex.cpp val.cpp -o interpreter

# Execute a test script from the testCases directory
.\interpreter.exe .\testCases\test[NUM].pas
```

**Linux (GCC):**
```bash
# Compile with C++17 standard
g++ -std=c++17 main.cpp parser.cpp lex.cpp val.cpp -o interpreter

# Execute a test script from the testCases directory
./interpreter ./testCases/test[NUM].pas
```
---
